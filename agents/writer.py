...
SYSTEM = (
    ...
    "- Gloss EVERY term a newcomer would not know, not just the obvious ones. "
    "pipeline, state machine, sandbox, schema, backend, resolution, "
    "orchestration, specification, checkpointer, verification: each of these "
    "means nothing on its own. 'a state machine (a set of steps, with rules "
    "for which one runs next)' is what a reader can use.\n"
    "- Never repeat a jargon-heavy sentence from the source. 'Handles the full "
    "dataset pipeline, including resolution, code generation and sandbox "
    "verification' is four unexplained terms in a row. Say what actually "
    "happens instead: 'Finds the dataset the paper used, writes the code to "
    "load it, then runs that code in a sealed container to check it works'.\n"
    ...
    "- Reciting call lists. 'It calls `a`, `b`, `c`, `d` and `e`' is the notes "
    "read aloud, and it tells a reader nothing. Name at most THREE functions "
    "in a paragraph, and only ones you actually say something about. The "
    "verified call list is there so you know what is true, not so you can "
    "print it.\n"
...
def write_overview(m: RepoMap, model: str = PROSE_MODEL) -> Section:
    ...
    prompt = (
        ...
        "Introduce this repository to someone who has never seen it and may "
        "never read its code. Answer their two questions, in this order: what "
        ...
        "- Then 1 or 2 sentences on the problem it solves. What would someone "
        "be doing by hand without it?\n"
        "- Then a short paragraph headed 'How it works': the whole thing start "
        "to finish, as a story. What goes in, what happens to it in order, "
        "what comes out. Three or four sentences. This is the part most "
        "readers want, so make it the clearest thing on the page.\n"
        "- Then the main parts and what each is FOR, one line each.\n"
        "\n"
        "The parts list is where jargon creeps back in, because the summaries "
        "you are given are written by and for the authors. Rewrite every one "
        "of them. A line like 'Manages the dataset pipeline, from resolution "
        "to verification' tells a newcomer nothing: it is three unexplained "
        "nouns. Write what the part actually does to something: 'Works out "
        "which dataset the paper used, then checks the code to load it really "
        "runs'. No line may contain an unexplained term.\n"
        "Give every part a plain-English name of your own, then the file, then "
        "what it does: '**Paper reader** (`arpa/tools/paper_extractor.py`): "
        "pulls the useful sections out of a paper and drops the rest'. A file "
        "path on its own is not a name, and the authors' title is often jargon "
        "too. 'Creates the pipeline state machine' and 'Defines the "
        "methodology specification' are both still unexplained: say 'sets out "
        "the order the steps run in' and 'the form filled in with what the "
        "paper said to build'.\n"
        "\n"
        "NO FUNCTION NAMES ANYWHERE IN THIS SECTION. Not one. Someone asking "
        "what a project is does not want `_extract_qwen_generation_text`. "
        "Describe the parts by what they do for a person: 'turns text into "
        "speech', 'writes the subtitles', 'stitches the video together'. Name "
        "the folder or file if it helps them find it, and nothing smaller.\n"
        "\n"
        "Under 200 words. Every statement must come from the facts above. "
        "No em dashes."
    )
